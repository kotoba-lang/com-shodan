(ns shodan.main
  "Kotodama WASM entrypoint for the Shodan clean-room actor (L4) — Clojure port.
  ...
  facts (`shodan.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "shodan")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"shodan.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "shodan-compat" :tier tier :entities entities} 200])
...
(defn start! [] :shodan-compat/ready)
